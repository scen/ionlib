...
public:
	...
	const std::string & GetChangedDirectoryName() const { return m_strChangedDirectoryName;}//WARNING: don't use this, this function will be removed in a future release.
...
protected:
	...
	virtual void On_FileAdded(const std::string & strFileName); //=0;
			//
			//	On_FileAdded()
			//
			...
			//
	virtual void On_FileRemoved(const std::string & strFileName);// = 0;
			//
			//	On_FileRemoved()
			//
			//	This function is called when a file in one of the watched folders(or subfolders)
	...
	virtual void On_FileNameChanged(const std::string & strOldFileName, const std::string & strNewFileName);// = 0;
			//
			//	On_FileNameChanged()
			//
			//	This function is called when a file in one of the watched folders(or subfolders)
	...
	virtual void On_FileModified(const std::string & strFileName);// = 0;
			//
			//	On_FileModified()
			//
	...
	virtual void On_ReadDirectoryChangesError(DWORD dwError, const std::string & strDirectoryName);
			//
	...
	virtual void On_WatchStarted(DWORD dwError, const std::string & strDirectoryName);
			//
			...
			//	const std::string & strDirectoryName -- The full path and name of the directory being watched.
	
	virtual void On_WatchStopped(const std::string & strDirectoryName);
			//
	...
	void SetChangedDirectoryName(const std::string & strChangedDirName);//please don't use this function, it will be removed in future releases.
	
private:
	long m_nRefCnt;
	
	std::string m_strChangedDirectoryName;//will be removed in a future release.
...
public:
	...
	//
	DWORD	WatchDirectory(const std::string & strDirToWatch, 
	...
	BOOL	IsWatchingDirectory (const std::string & strDirName)const;
	int		NumWatchedDirectories()const; //counts # of directories being watched.

	
	BOOL	UnwatchDirectory(const std::string & strDirToStopWatching);//stops watching specified directory.
...
private:
	...
	public:
		CDirWatchInfo(HANDLE hDir, const std::string & strDirectoryName, 
					  CDirectoryChangeHandler * pChangeHandler, 
	...
	public:
		...
		BOOL		m_bWatchSubDir;
		std::string     m_strDirName;//name of the directory that we're watching
	...
	//
	CDirWatchInfo *	GetDirWatchInfo(IN const std::string & strDirName, OUT int & ref_nIdx)const;
