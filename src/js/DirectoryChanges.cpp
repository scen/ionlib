...
/***********************************************************
  ...
  Latest Changes:
		...
		02/06/2002	  Fixed a bug that would cause an application to hang.
						...
						when it called CDirectoryChangeWatcher::UnwatchDirectory(const std::string & )
		...
		02/09/2002	Added a parameter to CDirectoryChangeHandler::On_ReadDirectoryChangeError()
					...
					Added the parameter: const std::string & strDirectoryName
					The new signature is now:
							virtual void CDirectoryChangeHandler::On_ReadDirectoryChangeError(DWORD dwError, const std::string & strDirectoryName);
		...
		04/25/2002	Added two new virtual functions to CDirectoryChangeHandler
					...
					Added:
							On_WatchStarted(DWORD dwError, const std::string & strDirectoryName)
							On_WatchStopped(const std::string & strDirectoryName);
		...
		04/27/2002  Added new Parameters to CDirectoryChangeWatcher::WatchDirectory()
					...
					Signature is now:
					CDirectoryChangeWatcher::WatchDirectory(const std::string & strDirToWatch, 
...
public:
	...
	std::string GetFileName()const;//gets the file name from the FILE_NOTIFY_INFORMATION record
	std::string GetFileNameWithPath(const std::string & strRootPath) const;//same as GetFileName() only it prefixes the strRootPath into the file name
...
std::string CFileNotifyInformation::GetFileName() const
{
	//
	//BUG FIX:
	...
	if( m_pCurrentRecord )
	{
		...
		return std::string( wcFileName );
	}
	return std::string();
}		

static inline bool HasTrailingBackslash(const std::string & str )
{
	if( str.GetLength() > 0 
...
std::string CFileNotifyInformation::GetFileNameWithPath(const std::string & strRootPath) const
{
	std::string strFileName( strRootPath );
...
//
//
void CDirectoryChangeHandler::On_FileAdded(const std::string & strFileName)
{ 
	TRACE(_T("The following file was added: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileRemoved(const std::string & strFileName)
{
	TRACE(_T("The following file was removed: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileModified(const std::string & strFileName)
{
	TRACE(_T("The following file was modified: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileNameChanged(const std::string & strOldFileName, const std::string & strNewFileName)
{
	TRACE(_T("The file %s was RENAMED to %s\n"), strOldFileName, strNewFileName);
}
void CDirectoryChangeHandler::On_ReadDirectoryChangesError(DWORD dwError, const std::string & strDirectoryName)
{
	TRACE(_T("WARNING!!!!!\n") );
...
void CDirectoryChangeHandler::On_WatchStarted(DWORD dwError, const std::string & strDirectoryName)
{	
	if( dwError == 0 )
...
void CDirectoryChangeHandler::On_WatchStopped(const std::string & strDirectoryName)
{
...
void CDirectoryChangeHandler::SetChangedDirectoryName(const std::string & strChangedDirName)
{
...
BOOL CDirectoryChangeWatcher::IsWatchingDirectory(const std::string & strDirName)const
/*********************************************
...
DWORD CDirectoryChangeWatcher::WatchDirectory(const std::string & strDirToWatch, 
									   DWORD dwChangesToWatchFor, 
									   CDirectoryChangeHandler * pChangeHandler,
									   BOOL bWatchSubDirs /*=FALSE*/,
									   LPCTSTR szIncludeFilter /*=NULL*/,
...
/*************************************************************
FUNCTION:	WatchDirectory(const std::string & strDirToWatch,   --the name of the directory to watch
...
PARAMETERS:
		const std::string & strDirToWatch -- specifies the path of the directory to watch.
...
BOOL CDirectoryChangeWatcher::UnwatchDirectory(const std::string & strDirToStopWatching)
/***************************************************************
FUNCTION:	UnwatchDirectory(const std::string & strDirToStopWatching -- if this directory is being watched, the watch is stopped
...
//
CDirectoryChangeWatcher::CDirWatchInfo * CDirectoryChangeWatcher::GetDirWatchInfo(const std::string & strDirName, int & ref_nIdx)const
{
		...
		case FILE_ACTION_RENAMED_OLD_NAME:
			{//a file name has changed, and this is the OLD name
			 ...
			 //the action set to FILE_ACTION_RENAMED_NEW_NAME (contains the new name of the file
				...
				std::string strOldFileName = notify_info.GetFileNameWithPath( pdi->m_strDirName );
				...
				{//there is another PFILE_NOTIFY_INFORMATION record following the one we're working on now...
				 //it will be the record for the FILE_ACTION_RENAMED_NEW_NAME record
					...
					std::string strNewFileName = notify_info.GetFileNameWithPath( pdi->m_strDirName );
